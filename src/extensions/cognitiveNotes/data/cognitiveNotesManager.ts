  ...
  private parseCognitiveNotesJsonV1(input: unknown): CognitiveNotesJson | null {
    ...
    const flowchart_nodes = rawFlowchartNodes
      ...
      .map((node) => {
        ...
        const createdOn =
          typeof (node as any).created_on === "string" ? (node as any).created_on : "";
        const updatedOn =
        ...
        return normalized as CognitiveNotesJson["flowchart_nodes"][number];
      })
    ...
    const rawFlowchartEdges = Array.isArray(obj.flowchart_edges)
      ...
      : [];
    const flowchart_edges = rawFlowchartEdges
